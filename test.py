...
cursor = conn.cursor()
# 创建pythonBD数据库
cursor.execute('drop DATABASE if EXISTS face_rec_2;')
cursor.execute('CREATE DATABASE face_rec_2;')
cursor.execute('use face_rec_2;')
...
image_path = "register_img/kang.jpg"
image = cv2.imread(image_path)
info = {"user_id": "10098440", "group_id": "stuff", "user_info": "康佳慧"}
...
if len(faces) != 1:
    ...
    cursor.execute(statement, (user_id, group_id, user_info, face_descriptor, image_path))
    conn.commit()

# cursor.execute("SELECT user_id, group_id, user_info, face_feature, image_path, latest_modify_time from users;")
# result = cursor.fetchall()
# # print(result)
# for user_id, group_id, user_info, face_feature, image_path, time in result:
#     # 将vector 从blob转换为numpy float16
#     face_descriptor_2 = np.fromstring(face_feature)
#     print(user_id, group_id, user_info, face_feature)

matched_id = 10098440
statement = """SELECT user_id, group_id, user_info, face_feature, image_path from users where user_id=%s;"""
